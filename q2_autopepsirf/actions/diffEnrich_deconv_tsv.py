...
def diffEnrich_deconv_tsv(
    ...
    raw_data = ctx.make_artifact(type="FeatureTable[RawCounts]",
                                 view=raw_data_tsv,
                                 view_type=PepsirfContingencyTSVFormat
                                 )

    bins = ctx.make_artifact(type="PeptideBins",
                             view=bins_tsv,
                             view_type=PeptideBinFormat
                             )

    if negative_control_tsv:
        negative_control = ctx.make_artifact(type="FeatureTable[Normed]",
    ...
    if thresh_file_tsv:
        thresh_file = ctx.make_artifact(type="EnrichThresh",
    ...
    linked = ctx.make_artifact(type="Link",
                               view=linked_tsv,
                               view_type=PepsirfLinkTSVFormat
                               )

    if id_name_map_tsv:
        id_name_map = ctx.make_artifact(type="PepsirfDMP",
    ...
    (col_sum, diff, diff_ratio, zscore_out, nan_out, sample_names, read_counts,
     ...
     colsum_scatter, zenrich_out, dir_out, score_per_round, map_dir
     ) = diffEnrich_deconv(raw_data = raw_data,
                           bins = bins,
    ...
    return (dir_out, score_per_round, map_dir, col_sum, diff, diff_ratio,
