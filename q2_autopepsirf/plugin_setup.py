...
from qiime2.plugin import (Plugin, TypeMap, Str, List, MetadataColumn,
...
shared_parameter_description = {
        "negative_id": "Optional approach for identifying negative controls. Provide a unique string at the start of all negative "
                       "control samples.",
        "negative_names": "Optional approach for identifying negative controls. Space-separated list of negative control sample "
                          "names.",
        "pepsirf_binary": "The binary to call pepsirf on your system.",
        "exact_z_thresh": "Individual Exact z score threshold separated by a comma for creation of threshold file to run pepsirf's "
                          "enrich module (Ex: 6,10 or 30)",
        "exact_cs_thresh": "Individual Exact col-sum threshold separated by a comma for creation of threshold file to run pepsirf's "
                           "enrich module (Ex: 6,10 or 30)",
        "raw_constraint": "The minimum total raw count across all peptides for a sample to be included in the analysis. This provides "                          "a way to impose a minimum read count for a sample to be evaluated.",
        "exact_zenrich_thresh": "List of exact z score thresholds either individual or combined. List MUST BE in descending order. "
                                "(Example argument: '--p-exact-zenrich-thresh 25 10 3' or '--p-exact-zenrich-thresh 6,25 4,10 1,3')",
        ...
        "pepsirf_tsv_dir": "Provide a directory path. Must also provide tsv-base-str for output of tsv verison of qza files. The "
                           "source_samples file and png boxplot outputs will always be put within this directory.",
        ...
        "hdi": "Alternative approach for discarding outliers prior to calculating mean and stdev. If provided, this argument will "
               "override --trim, which trims evenly from both sides of the distribution. For --hdi, the user should provide the high "
               "density interval to be used for calculation of mean and stdev. For example, '--hdi 0.95' would instruct the program "
               "to utilize the 95% highest density interval (from each bin) for these calculations.",
        "infer_pairs_source": "Infer sample pairs from names. This option assumes names of replicates will be identical with the "
                              "exception of a final string denoted with a '_'. For example, these names would be considered two "
                              "replicates of the same sample: VW_100_1X_A and VW_100_1X_B",
        "flexible_reps_source": "Will infer the number of replicates for each sample based on sample names, and will not require any "
                                "specific number of replicates for inclusion. Therefore, some samples may have a single replicate, "
                                "some may have 2, 3, 4 etc. And all replicates of a given sample will be considered for determining "
                                "enriched peptides.",
        "s_enrich_source": "All samples will be processed individually as samples with only one replicate",
        "user_defined_source": "Metadata file containing all sample names and their source groups. Used to create pairs tsv to run "
...
plugin.pipelines.register_function(
        ...
        input_descriptions={
            ...
            "thresh_file": "The name of a tab-delimited file containing one tab-delimited matrix filename and threshold(s), one per "
        ...
        description="Uses the diff normaization from pepsirf to generate Z scores that are used to determine enriched peptides."
)
...
plugin.pipelines.register_function(
        ...
        parameter_descriptions={
            ...
            "bins_filepath": "Name of the file containing bins, one bin per line, as output by the bin module. Each bin contains a "
                             "tab-delimited list of peptide names.",
            "thresh_file_filepath": "The name of a tab-delimited file containing one tab-delimited matrix filename and threshold(s), "
                                    "one per line. If providing more than z score matrix.",
            **shared_parameter_description
        },
...
plugin.pipelines.register_function(
        ...
        description="Uses the diff normalization from pepsirf to generate z scores that are used to determine enriched peptides and "
